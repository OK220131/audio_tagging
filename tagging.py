...
from panns_inference import models, pytorch_utils, config
"""
...
class AudioTagging(object):
    def __init__(self, model=None, checkpoint_path=None, device='cuda'):
        ...
        # Model
        if model is None:
            self.model = models.Cnn14(sample_rate=32000, window_size=1024, 
    ...
    def inference(self, audio):
        audio = pytorch_utils.move_data_to_device(audio, self.device)
...
class SoundEventDetection(object):
    def __init__(self, model=None, checkpoint_path=None, device='cuda', interpolate_mode='nearest'):
        ...
        # Model
        if model is None:
            self.model = models.Cnn14_DecisionLevelMax(sample_rate=32000, window_size=1024, 
                hop_size=320, mel_bins=64, fmin=50, fmax=14000, 
